...
class Point{
	...
	public Point duplicate() {
		return new Point(w, h, l);
	}

	@Override
	public String toString() {
		return "Point [w=" + w + ", h=" + h + ", l=" + l + "]";
	}
...
class Box{
	...
	public Box getSubBox(int len, Point vector) {
		Point s = start.duplicate();
		Point e = end.duplicate();
...
public class BJ1493_FillingBox {
	...
	static int recur(Box box) {
		...
		for(int d = 0; d < 7; d++) {
			Box next = box.getSubBox(len, new Point(dw[d],dh[d],dl[d]));
	...
	public static void main(String[] args) throws IOException {
		...
		System.out.println(recur(new Box(new Point(), new Point(W, H, L))));
	}
